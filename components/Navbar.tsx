...
import {
  Hotel, LayoutDashboard, BedDouble, CalendarCheck, Users, LogOut,
  ClipboardList, Globe, FileText, Menu, X, Heart, Search,
  Building2, Star, BarChart2, ShieldCheck,
} from "lucide-react";
  ...
  return (
    <header className="bg-white border-b border-slate-100 shadow-sm sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
        ...
        {/* Logo */}
        <Link href={session ? "/" : "/hotels"} className="flex items-center gap-2">
            ...
            <>
              ...
              <div className="hidden sm:flex items-center gap-2 ml-1">
                ...
                <div className="hidden md:block">
                  ...
                  <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${ROLE_BADGE[role]}`}>{role}</span>
                </div>
              </div>
              <button onClick={() => signOut({ callbackUrl: "/hotels" })}
          ...
          <nav className="px-4 py-3 space-y-1">
            ...
            <div className="border-t border-slate-100 pt-2 mt-2">
              <button onClick={toggleCalendar}
                ...
                Calendar: <strong>{isBS ? "Bikram Sambat" : "Gregorian"}</strong>
              </button>
              {session?.user ? (
                <button onClick={() => signOut({ callbackUrl: "/hotels" })}

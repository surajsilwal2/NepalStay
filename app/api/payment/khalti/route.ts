...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!process.env.KHALTI_SECRET_KEY) {
      return NextResponse.json({ success: false, error: "KHALTI_SECRET_KEY not configured" }, { status: 500 });
    }
    ...
    if (!["PENDING", "CONFIRMED"].includes(booking.status)) {
      return NextResponse.json({ success: false, error: "Booking cannot be paid in its current state" }, { status: 400 });
    }
    ...
    if (!kRes.ok) {
      ...
      return NextResponse.json({ success: false, error: `Khalti error (${kRes.status}): ${err}` }, { status: 502 });
    }

    const kData = await kRes.json();
    return NextResponse.json({
      success: true,
      data: { pidx: kData.pidx, payment_url: kData.payment_url, bookingId, amount: booking.totalPrice },
    });
  } catch (error) {

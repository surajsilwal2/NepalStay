...
const CITIES = ["All Cities", "Kathmandu", "Pokhara", "Chitwan", "Nagarkot", "Lumbini", "Bandipur"];
const PROPERTY_TYPES = ["All Types", "Hotel", "Guesthouse", "Resort", "Hostel", "Lodge", "Boutique Hotel"];
...
export default function HotelsClient() {
  ...
  const hasFilters =
    city !== "All Cities" || type !== "All Types" || stars > 0 || minPrice || maxPrice;
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          {compareList.length > 0 && (
            <p className="text-xs text-amber-600 font-medium flex items-center gap-1">
              ...
              {compareList.length} hotel{compareList.length > 1 ? "s" : ""} selected for comparison
            </p>
          )}
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                {hotels.map((hotel, idx) => (
                  <HotelCard
                    ...
                    onCompare={() => (isAdded(hotel.id) ? remove(hotel.id) : add(hotel as any))}
                    priority={idx < 3}
                  />
                ))}
              </div>
    ...
    <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden hover:shadow-md transition-shadow group relative">
      <button
        ...
        className={`absolute top-3 left-3 z-10 px-2 py-1 rounded-lg text-xs font-semibold backdrop-blur-sm transition-colors ${
          isInCompare ? "bg-amber-500 text-white" : "bg-white/90 text-slate-600 hover:bg-amber-50 hover:text-amber-700"
        }`}
      >
      ...
      <Link href={`/hotels/${hotel.slug}`} className="block">
        ...
        <div className="p-4">
          ...
          <div className="mb-2">
            <SeasonBadge basePrice={hotel.minPrice} showPriceDiff={false} compact />
          </div>
          ...
          <div className="flex items-center justify-between mt-2">
            <div>
              ...
              <span className="text-xl font-bold text-slate-800 ml-1">NPR {hotel.minPrice.toLocaleString()}</span>

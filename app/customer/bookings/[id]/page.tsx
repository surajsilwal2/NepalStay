...
type Booking = {
  ...
  hotel: { name: string; slug: string; city: string; address: string; images: string[]; contactPhone: string | null; contactEmail: string | null };
      ...
      <main className="max-w-2xl mx-auto px-4 py-8 print:px-0 print:py-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <p className="font-bold text-lg">
                  Nepal<span className="text-amber-400">Stay</span>
                </p>
                <p className="text-slate-400 text-xs">Booking Confirmation</p>
              </div>
            </div>
            <div
              className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-semibold ${cfg.cls}`}
            >
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex justify-between items-start flex-wrap gap-3">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Booked on{" "}
                  {isBS
                    ? formatBS(adToBS(new Date(booking.createdAt)))
            ...
            {/* QR Code — only show for confirmed/checked-in/checked-out with invoice */}
            {booking.invoiceNumber &&
            ...
            <div>
              ...
              <div className="mt-3 p-3 bg-slate-50 rounded-xl">
                ...
                <p className="text-xs text-slate-400">
                  {booking.room.type} · Floor {booking.room.floor} · NPR{" "}
                  {booking.room.pricePerNight.toLocaleString()}/night
                </p>
              </div>
            </div>
            ...
            <div className="bg-amber-50 rounded-xl p-4 border border-amber-100">
              <div className="flex justify-between text-sm text-slate-600 mb-1.5">
                <span>
                  NPR {booking.room.pricePerNight.toLocaleString()} ×{" "}
                  {booking.nights} night{booking.nights !== 1 ? "s" : ""}
                </span>
              ...
              >
                ...
                {(booking.refundAmount ?? 0) > 0 && (
                  <p className="text-sm mt-1">
                    ...
                    {booking.refundStatus === "COMPLETED"
                      ? "processed"
                      : "being processed (3–5 business days)"}
                  </p>
                )}
              </div>
            )}
            ...
            <div className="border-t border-slate-100 pt-4 flex items-center gap-2 text-xs text-slate-400">
              ...
              This booking is protected by the NepalStay guarantee. For support,
              contact {booking.hotel.contactEmail ?? "the hotel directly"}.
            </div>
          </div>
        </div>
